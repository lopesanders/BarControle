  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-2xl font-extrabold text-gray-800">Histórico</h3>
        {history.length > 0 && (
          <button 
            onClick={onClear}
        ...
        <div className="space-y-4">
          {history.map(session => (
            <button
              ...
              className="w-full text-left bg-white p-5 rounded-2xl shadow-sm border flex items-center justify-between group active:scale-[0.98] transition-all"
            >
              <div>
                ...
                <div className="flex items-center gap-3 mt-1 text-xs font-medium text-gray-500">
                  <span className="flex items-center gap-1"><Users size={12} /> {session.splitCount} pessoas</span>
                  {session.hasTip && <span className="flex items-center gap-1 text-green-600"><Percent size={12} /> Com gorjeta</span>}
                </div>
              </div>
      ...
      {selectedSession && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col max-h-[85vh]">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="p-4 bg-gray-50 rounded-2xl space-y-2">
                ...
                <div className="flex justify-between text-lg font-extrabold text-gray-900 pt-2 border-t border-gray-200">
                  ...
                  <span className="text-blue-600">{new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(selectedSession.total + selectedSession.tipAmount)}</span>
                </div>
              </div>
            ...
            <div className="p-6 bg-gray-50 border-t">
              <button 
                ...
                className="w-full py-4 bg-white border border-gray-200 text-gray-700 rounded-2xl font-bold shadow-sm"
              >
                Fechar
              </button>
            </div>
